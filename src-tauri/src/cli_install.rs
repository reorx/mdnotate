...
//!
//! What gets installed is a symlink to the binary bundled at
...
//! it is removing one link. `src-tauri/cli` is where that binary comes from and
//! what it does when run; nothing here ever runs it.
//!
...
/// The name the command goes by, which is also the name it is bundled under.
/// Not the cargo package's name — that one is `mdnotate-cli`, so as not to
/// collide with the app's own binary in the target directory.
...
pub enum LinkState {
    ...
    Missing,
    /// A symlink to this very app's command — the command is installed here.
    Ours,
...
pub enum AppLocation {
    ...
    Bundled,
    /// Not in a bundle: a `tauri dev` build, which has no command to link to.
    Unbundled,
...
/// Where the bundled command sits, given the app's resource directory.
pub fn command_path(resource_dir: &Path) -> PathBuf {
...
/// Where the app is, judged by the command it would be linking to. Told apart
...
pub fn app_location(command: &Path) -> AppLocation {
    if !command.is_file() {
    ...
    match command.canonicalize() {
        Ok(resolved) if resolved.starts_with("/Volumes/") => AppLocation::Removable,
        _ => AppLocation::Bundled,
    }
}

pub fn status(dirs: &[PathBuf], command: &Path) -> CliInstallStatus {
    CliInstallStatus {
        supported: true,
        app_location: app_location(command),
        entries: dirs.iter().map(|dir| entry(dir, command)).collect(),
    }
}

fn entry(dir: &Path, command: &Path) -> CliInstallEntry {
    let link = dir.join(COMMAND_NAME);
    CliInstallEntry {
        state: classify(&link, command),
...
fn classify(link: &Path, command: &Path) -> LinkState {
    ...
    // Both sides canonicalized, because `command` is only where we expect the
    // command to be, and the path it was built from need not be resolved yet.
    // With no command to compare against — a dev build — every link is somebody
...
/// Point `dir/mdnotate` at our own command, replacing a link already there.
///
...
pub fn install(dir: &Path, command: &Path) -> Result<(), String> {
    let link = dir.join(COMMAND_NAME);
    ...
    match link_directly(dir, &link, command) {
...
/// moment where the command points at neither the old command nor the new one.
fn link_directly(dir: &Path, link: &Path, command: &Path) -> std::io::Result<()> {
    ...
    let result = std::os::unix::fs::symlink(command, &staged).and_then(|_| std::fs::rename(&staged, link));
    if result.is_err() {
...
pub fn uninstall(dir: &Path, command: &Path) -> Result<(), String> {
    let link = dir.join(COMMAND_NAME);
    match classify(&link, command) {
...
fn escalated(shell_command: &str) -> Result<(), String> {
    let source = format!(
        "do shell command {} with administrator privileges",
...
mod tests {
    ...
    /// Stands in for the command inside the bundle.
    fn command_in(dir: &Path) -> PathBuf {
        let command = dir.join("app/Contents/Resources/bin/mdnotate");
    ...
    fn a_directory_without_the_command_reports_nothing_there() {
        let dir = scratch();
        assert_eq!(classify(&dir.join("mdnotate"), &command_in(&dir)), LinkState::Missing);
    }
    ...
    fn a_link_to_our_own_script_is_the_command_installed() {
        ...
        std::os::unix::fs::symlink(&command, dir.join("mdnotate")).unwrap();
        assert_eq!(classify(&dir.join("mdnotate"), &command), LinkState::Ours);
    }
    ...
    fn a_link_whose_app_is_gone_is_dangling_not_missing() {
        ...
        assert_eq!(classify(&dir.join("mdnotate"), &command_in(&dir)), LinkState::Dangling);
    }
    ...
    fn a_link_to_another_copy_of_the_app_is_foreign() {
        ...
        assert_eq!(classify(&dir.join("mdnotate"), &command_in(&dir)), LinkState::Foreign);
    }
    ...
    fn a_real_file_of_the_same_name_is_somebody_elses() {
        ...
        assert_eq!(classify(&dir.join("mdnotate"), &command_in(&dir)), LinkState::Occupied);
    }
    ...
    fn installing_where_we_have_no_permission_does_not_fail_outright() {
        ...
        let refused = link_directly(&locked, &locked.join("mdnotate"), &command)
    ...
    fn installing_makes_the_directory_it_needs() {
        ...
        install(&bin, &command).unwrap();
        assert_eq!(classify(&bin.join("mdnotate"), &command), LinkState::Ours);
    }
    ...
    fn installing_over_somebody_elses_file_is_refused_and_leaves_it_alone() {
        ...
        assert!(install(&dir, &command).is_err());
    ...
    fn uninstalling_what_is_not_there_is_not_an_error() {
        let dir = scratch();
        assert!(uninstall(&dir, &command_in(&dir)).is_ok());
    }
    ...
    fn uninstalling_takes_our_own_link_away() {
        ...
        let command = command_in(&dir);
        install(&dir, &command).unwrap();
        uninstall(&dir, &command).unwrap();
        assert_eq!(classify(&dir.join("mdnotate"), &command), LinkState::Missing);
    }
    ...
    fn uninstalling_refuses_a_link_that_is_not_ours() {
        ...
        std::fs::write(&other, "#!/bin/sh\n").unwrap();
        std::os::unix::fs::symlink(&other, dir.join("mdnotate")).unwrap();
        assert!(uninstall(&dir, &command).is_err());
    ...
    fn a_build_with_no_bundle_around_it_has_nothing_to_link_to() {
        let dir = scratch();
        assert_eq!(app_location(&dir.join("no-such-command")), AppLocation::Unbundled);
    }

    #[test]
    fn a_script_that_is_really_there_can_be_linked_to() {
        let dir = scratch();
        assert_eq!(app_location(&command_in(&dir)), AppLocation::Bundled);
    }
